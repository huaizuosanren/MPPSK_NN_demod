%function [snr,pe]= func_mppsk_demod_diffVd(Vd)
clc;
clear all;
close all;
Vd=5;
Vd
M=8   %4 % 64 %  %2  %4  %2  %16   %64    %128;     %128进制
...
%% ***********learn the signals' input and output under this SNR**************     
 % train_t=1:n1;
     if j==1
      net=elmannet(1:2,11,'trainscg');  %'traingdx');
      net.trainParam.epochs=5000;  % iterations
      net.trainParam.lr=0.001;  % learning ratio
      net.trainParam.goal=0;  %0.00000001;  % mu biao wu cha
